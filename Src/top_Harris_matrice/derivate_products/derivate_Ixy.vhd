...
entity derivate_Ixy is
    port (
        clk           : in  std_logic;
        reset        : in  std_logic;
        valid_in_x   : in  std_logic;
        valid_in_y   : in  std_logic;
...
architecture Behavioral of derivate_Ixy is
    ...
    type state_type is (WAITING, COMPUTE, OUTPUT);
            ...
            if rising_edge(clk) then
                if reset = '1' then
                    ...
                    case state is
                        ...
                        when WAITING =>
                            valid_out <= '0';
                            if valid_in_x = '1' then
                                Ix1_reg <= signed(Ix1);
                                Ix2_reg <= signed(Ix2);
                                Ix3_reg <= signed(Ix3);
                                flag_X <= '1';
                            end if;
                            if valid_in_y = '1' then
                                Iy1_reg <= signed(Iy1);
                                Iy2_reg <= signed(Iy2);
                                Iy3_reg <= signed(Iy3);
                                flag_Y <= '1';
